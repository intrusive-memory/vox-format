...
```json
{
  ...
  "reference_audio": [
    ...
    {
      "file": "reference/sample-02-qwen3.wav",
      "transcript": "En este tiempo, vino a posar al mesón un ciego.",
      "language": "es",
      "duration_seconds": 3.8,
      "model": "Qwen/Qwen3-TTS-12Hz-1.7B",
      "engine": "qwen3-tts",
      "context": "Model-generated sample for Qwen3-TTS quality matching."
    }
  ],
...
#### Per-Model Reference Audio (v0.3.0)

Reference audio clips may optionally be tagged with the model and engine that produced or are intended for them:

| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `file` | Yes | string | Archive-relative path (e.g., `"reference/sample-01.wav"`). |
| `transcript` | Yes | string | Verbatim transcript of the audio clip. |
| `language` | No | string | BCP 47 language code (e.g., `"en-US"`). |
| `duration_seconds` | No | number | Duration in seconds. |
| `context` | No | string | Contextual note about the clip. |
| `model` | No | string | Model identifier this clip was produced by or intended for. Should match an `embeddings` entry's `model` value. |
| `engine` | No | string | Engine namespace (e.g., `"qwen3-tts"`). |

Clips without `model` or `engine` are "universal" — usable by any engine. When multiple clips exist, consumers should prefer model-matched clips and fall back to universal clips if no match is found.
...
|----------------|---------|
| `designed` | Generated from a text description only (no real person, no audio generated). |
| `synthesized` | A model generated actual audio from a voice design (distinct from `designed` which is text-only). |
...
The `source` field (v0.3.0):
- Type: `[string]` — Array of archive-relative paths to source audio files used for cloning.
- Required when `method` is `"cloned"` (see Ethical Requirements below).
- Example: `["reference/source-recording-01.wav", "reference/source-recording-02.wav"]`

### Ethical Requirements (v0.3.0)

The following rules enforce ethical guardrails around voice cloning:

**MUST (validation errors if violated):**

1. **Cloned voices require source traceability.** When `provenance.method` is `"cloned"`, the `provenance.source` array MUST be present and contain at least one entry. This ensures every cloned voice can be traced back to its source material.

2. **Cloned voices require explicit consent.** When `provenance.method` is `"cloned"`, the `provenance.consent` field MUST be `"self"` or `"granted"`. Values of `"unknown"` or `null` are validation errors for cloned voices.

3. **Model-generated audio must use `"synthesized"`.** When a TTS model has generated actual audio samples (not just a text description), use `method: "synthesized"` rather than `"designed"`. The `"designed"` method is reserved for text-description-only voice identities where no audio was generated by a model.

**SHOULD (validation warnings if violated):**

4. **Model-tagged reference audio should have a corresponding embedding.** If a `reference_audio` entry has a `model` field set, there should be a matching entry in the `embeddings` section. A mismatch suggests the manifest is incomplete.
