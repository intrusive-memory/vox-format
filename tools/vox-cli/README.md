...
`vox-cli` provides a set of commands for creating, inspecting, validating, and extracting VOX voice identity archives. VOX is an open, vendor-neutral file format for voice identities used in text-to-speech synthesis. This tool makes it easy to work with `.vox` files during development, testing, and production workflows.

The CLI supports all features of the VOX format specification including voice metadata, prosody preferences, character context, reference audio, provenance tracking, and engine-specific extensions.

## Installation

Build the CLI tool from source using Swift Package Manager:
...
The compiled binary will be located at `.build/release/vox-cli`.

Optionally, install the binary to your PATH:

```bash
# Copy to /usr/local/bin (requires sudo)
sudo cp .build/release/vox-cli /usr/local/bin/

# Or add to your shell profile
export PATH="/path/to/vox-format/tools/vox-cli/.build/release:$PATH"
```
...
```bash
vox-cli validate [--strict] <file.vox>
```
...
vox-cli validate --strict examples/character/protagonist.vox
```
...
```bash
vox-cli create --name <name> --description <description> --output <file.vox> [options]
```
...
```bash
vox-cli extract <file.vox> --output-dir <directory>
```

**Example:**

```bash
vox-cli extract examples/character/protagonist.vox --output-dir extracted/
vox-cli extract narrator.vox --output-dir narrator-contents/
```
...
3. **Validate the result**:
   ...
   ```bash
   vox-cli validate protagonist.vox
   ```

4. **Inspect the contents**:

   ```bash
   vox-cli inspect protagonist.vox
   ```
...
3. **For development, use strict mode**:
   ...
   ```bash
   vox-cli validate --strict voice.vox
   ```
...
2. **Extract to examine raw files**:
   ...
   ```bash
   vox-cli extract voice.vox --output-dir voice-contents/
   cd voice-contents/
   cat manifest.json
   ls -la reference/
   ```
...
```bash
vox-cli extract voice.vox --output-dir extracted/
...
- [VoxFormat Swift Library](../../implementations/swift/README.md)
